...
const UpcomingEvent = () => {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flexWrap:"wrap",marginTop:"80px",justifyContent:"center"}}>
      ...
      <Typography variant="h5"style={{ fontFamily: 'Arial, sans-serif',fontWeight: 'bold',fontSize: '30px',color: 'rgb(247, 158, 2)',textAlign: 'center', marginBottom: '24px',}}>View Result of Past Event's</Typography>
      <div style={{display:"flex",flexDirection:"row",flexWrap:"wrap",justifyContent:"center" }}>
      {events.map(event => (
        (currentDate>event?.Eventdate && <UpcomingEventhelper  event={event} />)
       ))}
